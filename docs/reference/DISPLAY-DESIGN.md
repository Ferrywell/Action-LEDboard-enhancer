...
**Standaardkleuren (tekst):** `renderLines` gebruikt standaard **amber** RGB(255, 153, 0) / `#ff9900` op **zwart** — hoog contrast op het paneel, conform bovenstaande richtlijn. Aanroepers kunnen `foreground`/`background` nog steeds overschrijven. **`adjustPNG`** zet geen tekstkleur: het schaalt/rotereert alleen de aangeleverde bitmap en past helderheid toe; kleur zit in de pixeldata.

**Nog niet of afwijkend:** er is **geen vaste 5×7 dot-matrix**; er wordt `UIFont.monospacedSystemFont(ofSize: 5, weight: .medium)` gebruikt. Daarmee ontbreekt de “LED-segment”-look en is het maximum aantal leesbare tekens per rij niet hetzelfde als bij `CHAR_WIDTH`/`scale` in Python — korte monospace-strings blijven nodig. Er is **geen harde limiet** op het aantal regels in de API; te veel regels worden door layout klein of afgekapt. **GIF** wordt in deze renderer niet opgebouwd; animatie/scroll blijft buiten dit bestand (ontwerpkeuze: GIF elders of server-side).
